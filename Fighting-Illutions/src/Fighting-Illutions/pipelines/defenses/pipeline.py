...
def create_pipeline(**kwargs) -> Pipeline:
    ...
    for index, model_ref in enumerate(Models):
        for attack_type in Attacks:
            ...
            output_result.update({"KL_report_{defense}":f"{model_ref}_KLdiff_{defense}_{attack_type}@Report" for defense in Defenses})
            output_result.update({"Conf_matrix_{defense}":f"{model_ref}_confusionMatrix_{defense}_{attack_type}@Report" for defense in Defenses})
            output_result.update({"Distributions_{defense}":f"{model_ref}_Distribution_{defense}_{attack_type}@Report" for defense in Defenses})
